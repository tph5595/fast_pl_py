...
#[pyfunction]
fn pairs_to_landscape(bd_pairs: Vec<(f64, f64)>, k: usize, debug: bool) -> PyResult<Vec<Vec<(f64, f64)>>> {
...
#[pyfunction]
fn pairs_to_l2_norm(bd_pairs: Vec<(f64, f64)>, k: usize, debug: bool) -> PyResult<f64> {
    let bd_pairs = convert_bd_pairs(bd_pairs);
